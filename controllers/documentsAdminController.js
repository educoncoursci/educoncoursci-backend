// ============================================================
//  controllers/documentsAdminController.js
//  Gère : génération et export des documents administratifs
//  (Demande, Attestation, Courrier, Compte rendu, Procès-verbal)
// ============================================================

const {
  genererDemande, genererAttestation, genererCourrier,
  genererCompteRendu, genererProcesVerbal,
} = require("../services/documentsAdminClaude");
const { genererPDFTexte, supprimerFichier } = require("../services/pdf");
const { genererDocumentDocx } = require("../services/docx");
const { listerTypesAdmin, obtenirTypeAdmin } = require("../config/typesDocumentsAdmin");

const GENERATEURS = {
  demande: genererDemande,
  attestation: genererAttestation,
  courrier: genererCourrier,
  compte_rendu: genererCompteRendu,
  proces_verbal: genererProcesVerbal,
};

// ════════════════════════════════════════════════════════════
//  GET /api/documents-admin/types — Liste des types (public)
// ════════════════════════════════════════════════════════════
exports.listerTypes = (req, res) => {
  try {
    res.json({ types: listerTypesAdmin() });
  } catch (err) {
    console.error("Erreur liste types documents admin :", err.message);
    res.status(500).json({ error: "Erreur lors de la récupération des types de documents." });
  }
};

// ════════════════════════════════════════════════════════════
//  POST /api/documents-admin/generate — Générer un document
// ════════════════════════════════════════════════════════════
exports.genererDocument = async (req, res) => {
  try {
    ...
    const typeInfo = obtenirTypeAdmin(type);
    ...
    console.error("Erreur génération document admin :", err.message);
...
// ════════════════════════════════════════════════════════════
//  POST /api/documents-admin/pdf — Exporter en PDF
// ════════════════════════════════════════════════════════════
exports.exportPDF = async (req, res) => {
  ...
  try {
    ...
    console.error("Erreur export PDF document admin :", err.message);
...
// ════════════════════════════════════════════════════════════
//  POST /api/documents-admin/docx — Exporter en Word
// ════════════════════════════════════════════════════════════
exports.exportDOCX = async (req, res) => {
  ...
  try {
    ...
    console.error("Erreur export DOCX document admin :", err.message);
    if (filePath) supprimerFichier(filePath);
