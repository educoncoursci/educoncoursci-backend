...
// date_cloture est la vraie colonne DATE (ISO) — cloture est désormais
// du texte formaté en français ("15 janvier 2026"), que new Date() ne
// parse pas correctement ; on la garde seulement pour l'affichage dans
// les messages envoyés plus bas.
const result = await query(`SELECT * FROM concours WHERE statut = 'ouvert' AND date_cloture IS NOT NULL`);

const concoursAlertes = [];

for (const concours of result.rows) {
  const dateCloture = new Date(concours.date_cloture);
